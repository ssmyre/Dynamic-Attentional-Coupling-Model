...
class MultiTransModelContext_Reduced(nn.Module):
    def __init__(self, hyp_params, writer=None):
        ...
        # 4. Combine Self and Crossmodal Attentions
        self.trans_a_combine = self.get_transformer(self_type='combined_mem', layers=self.layers)
        self.trans_v_combine = self.get_transformer(self_type='combined_mem', layers=self.layers)

        # Projection layers
        self.proj1 = nn.Linear(2*combined_dim, 2*combined_dim)
        self.proj2 = nn.Linear(2*combined_dim, 2*combined_dim)
        self.final_fc = nn.Linear(2*combined_dim, self.z_dim)
    ...
    def _vis_network(self):
        ...
        self.v_conv1 = nn.Conv2d(3, 4, 3,1,padding=1)
        ...
        self.v_convt9 = nn.ConvTranspose2d(4,3,3,1,padding=1)
    ...
    def _get_layers(self):
        ...
        return {'a_fc1':self.a_fc1, 'a_fc21':self.a_fc21, 'a_fc22':self.a_fc22, 'a_fc23':self.a_fc23, 
                ...
                'trans_a_with_v':self.trans_a_with_v,  'trans_v_with_a':self.trans_v_with_a, 
                'trans_a_mem':self.trans_a_mem, 'trans_v_mem':self.trans_v_mem, 
                'proj1':self.proj1, 'proj2':self.proj2, 'final_fc':self.final_fc}
    ...
    def decode_v(self, z):
        ...
        # print(z.view(-1, X_V_DIM,3).shape)
        return z.view(-1, X_V_DIM,3)
    
    def forward(self, batch, xa, xa_ns, xv, xv_ns, return_latent_rec=False):
        """
        ...
        # print(mu_a.shape)
        mu_a = torch.stack(torch.split(mu_a, int(mu_a.shape[0]/2))) #reshape to be (Batch x Context x D) (2x12x64)
        ...
        mu_v = torch.stack(torch.split(mu_v, int(mu_v.shape[0]/2))) #reshape to be (Batch x Context x D) (2x12x64)
        # print('zv', zv.shape)
        
        #Auditory Mode
        if self.training_mode[batch] == 0:
            # (V) --> A
            h_as = self.trans_a_mem(mu_a) #self attention
            h_a_with_vs = torch.zeros(mu_a.shape)
            h_as_com = torch.cat([h_as, h_a_with_vs], dim=2) 
            h_as_com = self.trans_a_combine (h_as_com) #cross-modal
            if type(h_as_com) == tuple:
                h_as_com = h_as_com[0]

            last_h_a = torch.reshape(h_as_com,(-1,h_as_com.shape[2])) #reshape (BCxD)
            last_h_v = torch.zeros(last_h_a.shape)
            last_hs  = torch.cat([last_h_a, last_h_v], dim=1)

        #Visual Mode
        if self.training_mode[batch] == 1:
            # (A) --> V
            h_vs = self.trans_v_mem(mu_v)
            h_v_with_as = torch.zeros(mu_v.shape)
            h_vs_com = torch.cat([h_vs, h_v_with_as], dim=2)
            h_vs_com = self.trans_a_combine (h_vs_com)
            if type(h_vs_com) == tuple:
                h_vs_com = h_vs_com[0]
            last_h_v = torch.reshape(h_vs_com,(-1,h_vs_com.shape[2])) #reshape (BCxD)
            last_h_a = torch.zeros(last_h_v.shape)
            last_hs  = torch.cat([last_h_a, last_h_v], dim=1)

        #Crossmodal Mode
        if self.training_mode[batch] == 2:
            h_as = self.trans_a_mem(mu_a) #self attention
            h_a_with_vs = self.trans_a_with_v(mu_a, mu_v, mu_v) #cross-modal attention
            h_as_com = torch.cat([h_as, h_a_with_vs], dim=2) #Concat self x cross
            h_as_com = self.trans_a_combine (h_as_com) # self.attention
            if type(h_as_com) == tuple:
                h_as_com = h_as_com[0]
            last_h_a = last_hs = torch.reshape(h_as_com,(-1,h_as_com.shape[2])) #reshape (BCxD) Audio transformer output

            # (A) --> V
            h_vs = self.trans_v_mem(mu_v) #self attention
            h_v_with_as = self.trans_v_with_a(mu_v, mu_a, mu_a) #cross-modal attention
            h_vs_com = torch.cat([h_vs, h_v_with_as], dim=2) #Concat self x cross
            h_vs_com = self.trans_v_combine (h_vs_com)  # self.attention

            if type(h_vs_com) == tuple:
                h_vs_com = h_vs_com[0]
            last_h_v = last_hs = torch.reshape(h_vs_com,(-1,h_vs_com.shape[2])) #reshape (BCxD)  Video transformer output

            last_hs = torch.cat([last_h_a, last_h_v], dim=1) #Concat 
        # print(last_hs.shape)

        # A residual block
        last_hs_proj = self.proj2(F.dropout(F.relu(self.proj1(last_hs)), p=self.out_dropout, training=self.training))
        last_hs_proj += last_hs
        last_hs_fc = self.final_fc(last_hs_proj)


        rec_a =  self.decode_a(last_hs_fc) #reconstruct original audio image
        rec_v =  self.decode_v(last_hs_fc) #reconstruct original video image        
        loss_a= torch.mean((xa_ns.view(xa_ns.shape[0],-1) - rec_a) ** 2)
        loss_v= torch.mean((xv_ns.view(xv_ns.shape[0],-1, 3) - rec_v) ** 2)
        ...
        if return_latent_rec:
            return total_loss, last_hs_fc.detach().cpu().numpy(), mu_a.detach().cpu().numpy(), mu_v.detach().cpu().numpy(), \
                rec_a.view(-1, X_A_SHAPE[0], X_A_SHAPE[1]).detach().cpu().numpy(), \
                rec_v.view(-1, 3, X_V_SHAPE[0], X_V_SHAPE[1]).detach().cpu().numpy()
        return total_loss
    ...
    def train_epoch(self, loader):
        ...
        for i_batch, data in enumerate(loader):
            ...
            loss = self.forward(i_batch, audio, audio_ns, vision, vision_ns)
        ...
        #         train_loss))
        print('Epoch: {:2d} | Time/Epoch(ms): {:5.2f} | Average Loss: {:.4f}'.
                format(self.epoch, self.elapsed_time * 1000, train_loss))
        self.epoch += 1
        return train_loss
    ...
    def train_loop(self, train_loader, test_loader, hyp_params, epochs=100, training_prob=[0, 0, 1],
        ...
        for epoch in range(self.epoch, self.epoch+epochs):
            ...
            self.training_mode = np.random.choice([0, 1, 2], size=len(train_loader.dataset), p=training_prob)  #Here 0: Auditory, 1: Visual, 2: Crossmodal
    ...
    def load_state(self, filename, save_dir=''):
        ...
        self.elapsed_time = checkpoint['Time_Epoch']
    ...
    def save_state(self, filename):
        ...
        state = {}
        for layer_name in layers:
            if layers[layer_name] is None: continue
        ...
        state['lr'] = self.lr
        state['Time_Epoch'] = self.elapsed_time
    ...
    def check_latents(self, batch, xa, xv):

        with torch.no_grad():

            mu_a_o, u_a, d_a = self.encode_a(xa)
            mu_a = torch.stack(torch.split(mu_a_o, int(mu_a_o.shape[0]/2))) #reshape to be (Batch x Context x D) (2x12x64)

            mu_v_o, u_v, d_v = self.encode_v(xv)
            mu_v = torch.stack(torch.split(mu_v_o, int(mu_v_o.shape[0]/2))) #reshape to be (Batch x Context x D) (2x12x64)            
        ...
        return(mu_a_o.detach().cpu().numpy(), mu_v_o.detach().cpu().numpy(), last_hs_fc.detach().cpu().numpy())
    ...
    def visualize(self, loader, num_specs=5, gap=(2,6), \
        ...
        # Get reconstructions.
        with torch.no_grad():
            _,_,_,_, aud_rec_specs, vis_rec_specs = self.forward(audio, audio_ns, vision, vision_ns, return_latent_rec=True)
